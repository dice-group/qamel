...
public class Genesis {
    ...
    public Genesis() {
        ...
        /*try {
            // Initialize CouchDB
            cloudantClient = ClientBuilder.url(new URL(couchDBUrl))
                    .username(couchDBUsername)
                    .password(couchDBPassword)
                    .build();

            explorerDB =  cloudantClient.database(couchDBName, false);
            process();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public void process() {
        ...
        System.out.println("get all ontology classes and properties"+queryExecution);
        try {
            ...
            while(results.hasNext()) {
                ...
                if(solution.get("types") != null && solution.get("properties") != null) {
                    ...
                    List<String> fields = getRelevantProperties(uri, types, properties); // Get Relevant Properties based on CouchDB
                    if(fields.size() > 0) {
    ...
    public  List<String> getRelevantProperties(String uri,List Answer, String[] properties) {
