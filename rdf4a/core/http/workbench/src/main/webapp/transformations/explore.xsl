				...
				<table class="dataentry">
					<tbody>
						<tr>
							...
							<td>
								<xsl:if
									test="count(//sparql:binding[@name='error-message']) &gt; 0">
									<span class="error">
										<xsl:value-of
						...
						<tr>
							<th>
								<xsl:value-of
									select="$result-limit.label" />
							</th>
							<td>
								<xsl:call-template
									name="limit-select">
									<xsl:with-param name="onchange">
										workbench.paging.addLimit('explore');
									</xsl:with-param>
									<xsl:with-param name="limit_id">
										limit_explore
									</xsl:with-param>
								</xsl:call-template>
							</td>
						...
						<tr>
							<th>
								<xsl:value-of
									select="$result-offset.label" />
							</th>
							<td>
								<input id="previousX" type="button"
									value="{$previousX.label}"
									onclick="workbench.paging.previousOffset('explore');" />
							</td>
							<td>
								<input id="nextX" type="button"
									value="{$nextX.label}"
									onclick="workbench.paging.nextOffset('explore');" />
							</td>
						</tr>
						<tr>
							<th>
								<xsl:value-of
									select="$show-datatypes.label" />
							</th>
							<td>
								<input type="checkbox"
									name="show-datatypes" value="show-dataypes" checked="checked" />
							</td>
						</tr>
					</tbody>
				</table>
